...
class WP_Stream_Settings {
	...
	public static function load() {
		...
		// update_option('wp_stream_site_uuid','RWKT327V1X0XXASVP7HL1ZMU');
		// update_option('wp_stream_site_api_key','The only key youll ever need!');
	}
	...
	*/
	...
	public static function get_fields() {
			...
			'general' => array(
				...
				'fields' => array(
					array(
						...
						'type'        => 'multi_checkbox',
						'desc'        => esc_html__( 'Users from the selected roles above will have permission to view Signal Records. However, only site Administrators can access Signal Settings.', 'stream' ),
					...
					array(
						...
						'type'        => 'checkbox',
						'desc'        => sprintf(
							__( 'Users from the selected roles above will be given a private key found in their %suser profile%s to access feeds of Signal Records securely. Please %sflush rewrite rules%s on your site after changing this setting.', 'stream' ),
							sprintf(
								'<a href="%s" title="%s">',
			...
			'exclude' => array(
				...
				'fields' => array(
					array(
						...
						'type'        => 'rule_list',
						'desc'        => esc_html__( 'Create rules for excluding certain kinds of records from appearing in Signals.', 'stream' ),
						'default'     => array(),
			...
			'advanced' => array(
				...
				'fields' => array(
					array(
						...
						'type'        => 'checkbox',
						'desc'        => __( 'WordPress will automatically prevent duplicate comments from flooding the database. By default, Signals does not track these attempts unless you opt-in here. Enabling this is not necessary or recommended for most sites.', 'stream' ),
						'after_field' => esc_html__( 'Enabled', 'stream' ),
						'default'     => 0,
					),
				),
		...
		if ( class_exists( 'Akismet' ) ) {
			$akismet_tracking = array(
				...
				'type'        => 'checkbox',
				'desc'        => __( 'Akismet already keeps statistics for comment attempts that it blocks as SPAM. By default, Signals does not track these attempts unless you opt-in here. Enabling this is not necessary or recommended for most sites.', 'stream' ),
				'after_field' => esc_html__( 'Enabled', 'stream' ),
				'default'     => 0,
			);
		...
		if ( wp_stream_is_wp_cron_enabled() ) {
			$wp_cron_tracking = array(
				...
				'type'        => 'checkbox',
				'desc'        => __( 'By default, Signals does not track activity performed by WordPress cron events unless you opt-in here. Enabling this is not necessary or recommended for most sites.', 'stream' ),
				'after_field' => esc_html__( 'Enabled', 'stream' ),
				'default'     => 0,
			);
	...
	public static function render_field( $field ) {
		...
		switch ( $type ) {
			...
			case 'button':
				$output = sprintf(
					'<input class="button button-primary" value="$s" type="submit" name="$s">',
					esc_attr( $name ),
					esc_attr( $name )
				);
				break;
