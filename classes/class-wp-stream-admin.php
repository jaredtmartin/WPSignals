...
class WP_Stream_Admin {
	...
	// const WP_Stream::$api->api_url              = 'http://104.236.54.170';
	...
	public static function load() {
		...
		self::$connect_url = add_query_arg(
			array(
				...
				'api_key'     => WP_Stream::$api->api_key,
				'back'   => site_url(),
				'nonce' => $connect_nonce,
			),
			esc_url_raw( untrailingslashit( WP_Stream::$api->api_url ) . '/domains/create/' )
		);
		...
		<div id="stream-message" class="updated stream-connect" style="display:block !important;">
			...
			<div class="stream-message-container">
				...
				<div class="stream-button-container">
					<a href="<?php echo esc_url( self::$connect_url ) ?>" class="stream-button"><i class="stream-icon"></i><?php _e( 'Connect to WP Signals', 'stream' ) ?></a>
				</div>

				<div class="stream-message-text">
					<h4><?php esc_html_e( 'WP Signals is almost ready!', 'stream' ) ?></h4>
					<p>
						<?php
						echo wp_kses_post(
							sprintf(
								esc_html__( 'Connect to WP Signals to see every change made to your site in beautifully organized detail.', 'stream' ),
			...
			case 'connected':
				if ( ! WP_Stream_Migrate::show_migrate_notice() ) {
					$notice = sprintf(
						...
						esc_html__( 'You have successfully connected to WP Signals!', 'stream' ),
	...
	public static function account_url( $path = '' ) {
		$account_url = add_query_arg(
			...
			esc_url_raw( sprintf( '%s/dashboard/%s', untrailingslashit( WP_Stream::$api->api_url ), untrailingslashit( $path ) ) )
		);
	...
	public static function register_menu() {
		// error_log("WP_Stream::is_development_mode(): ".WP_Stream::is_development_mode());
		// error_log("WP_Stream::is_connected(): ".WP_Stream::is_connected());
		if ( WP_Stream::is_connected() || WP_Stream::is_development_mode() ) {
			$unread_count = self::get_unread_count();
			$menu_title   = __( 'WP Signals', 'stream' );
			...
			self::$screen_id['connect'] = add_menu_page(
				__( 'Connect to WP Signals', 'stream' ),
				__( 'WP Signals', 'stream' ),
				self::SETTINGS_CAP,
				self::ACCOUNT_PAGE_SLUG,
	...
	public static function save_api_authentication() {
		...
		if ( ! isset( $_GET['api_key'] ) || ! isset( $_GET['site_uuid'] ) ) {
			wp_die( 'There was a problem connecting to WP Signals. Please try again later.', 'stream' );
		}
		...
		if ( ! isset( $site->site_id ) ) {
			wp_die( 'There was a problem verifying your site with WP Signals. Please try again later.', 'stream' );
		}
	...
	public static function get_testimonials() {
		...
		$url     = sprintf( '%s/wp-content/themes/wp-stream.com/assets/testimonials.json', untrailingslashit( WP_Stream::$api->api_url ) );
	...
	public static function render_account_page() {
		...
		if (false) {
			...
			?>
				...
				<form id="account_form" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ) ?>" method="post">
					<div class="plan-details">
						<table class="form-table">
							<tbody>
								<tr>
								...
								<tr>
									<th><?php _e( 'Site UUID', 'stream' ) ?></th>
									<td><?php echo esc_html( $site_uuid ) ?></td>
									<!-- <td> -->
										<!-- <input type="text" name="api_key" value="<?php echo esc_html( $api_key ) ?>" size="40" /> -->
									<!-- </td> -->
								</tr>
							</tbody>
						</table>
					...
					<div class="plan-actions submitbox">
						<a class="button button-danger button-large" href="<?php echo $activity_url ?>">See Activity</a>
						<input type="submit" class="button button-danger button-large" name="clear_key" value="Disconnect">
						<?php echo $submitted ?>
					</div>
				</form>
			<?php
	...
	public static function render_connect_page() {
		...
		// $connectUrl = "http://localhost:3000/create_domain?name=$blog_title&back=$blog_url&api_key=$api_key";
		wp_enqueue_style( 'wp-stream-connect', WP_STREAM_URL . 'ui/css/connect.css', array(), WP_Stream::VERSION );
		wp_enqueue_script( 'wp-stream-connect', WP_STREAM_URL . 'ui/js/connect.js', array(), WP_Stream::VERSION );
		?>
	...
	public static function render_stream_page() {
		$page_title = __( 'WP Signals Records', 'stream' );

		self::$list_table->prepare_items();

		echo '<div class="wrap">';

		printf( '<h2>%s</h2>', __( 'WP Signals Records', 'stream' ) ); // xss ok
